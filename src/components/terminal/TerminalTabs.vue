...
<template>
  ...
  >
    ...
    >
      <!-- Cloud type icon -->
      <svg v-if="sessionStore.sessions.get(tab.sessionId)?.type === 'kube-exec' || sessionStore.sessions.get(tab.sessionId)?.type === 'kube-logs'" class="w-3 h-3 shrink-0" viewBox="0 0 24 24" fill="#326CE5">
        <path d="M12 2L3.5 7v10L12 22l8.5-5V7L12 2zm0 2.18L18.5 8v8L12 19.82 5.5 16V8L12 4.18z"/>
        <circle cx="12" cy="12" r="3" />
      </svg>
      <svg v-else-if="sessionStore.sessions.get(tab.sessionId)?.type === 'ssm'" class="w-3 h-3 shrink-0" viewBox="0 0 24 24" fill="#FF9900">
