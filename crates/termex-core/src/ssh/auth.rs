...
pub struct ClientHandler {
    ...
    /// Event emitter for port-forward notifications (replaces tauri::AppHandle).
    pub event_emitter: Option<BoxedEmitter>,
...
impl client::Handler for ClientHandler {
    ...
    /// Reads the HTTP request from the remote curl, parses it, and emits an event.
                ...
                match channel.wait().await {
                    Some(ChannelMsg::Data { data }) => {
                        ...
                        if let Some(action) = parse_sync_request(&buf) {
                            ...
                            match action {
                                SyncAction::PushDone => {
                                    emitter.emit_port_forward_event("git-sync://push-done", &server_id);
                                }
                            }
                            return;
                        }
