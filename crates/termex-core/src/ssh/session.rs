...
impl SshSession {
    ...
    /// Clones the shell channel's command sender.
    ///
    /// Returns `None` if `open_shell` has not yet been called. The clone
    /// can be stored in an external registry to issue writes/resizes
    /// without borrowing the session back.
    pub fn channel_cmd_tx(
        &self,
    ) -> Option<tokio::sync::mpsc::UnboundedSender<ChannelCommand>> {
        self.channel.as_ref().map(|c| c.cmd_tx.clone())
    }
    ...
    pub async fn open_shell(
        ...
        let handle = spawn_channel_task(channel, emitter, session_id);
