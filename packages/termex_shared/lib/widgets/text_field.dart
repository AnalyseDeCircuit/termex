...
class _TermexTextFieldState extends State<TermexTextField> {
  ...
  void initState() {
    ...
    _placeholderShown = widget.placeholder != null && _controller.text.isEmpty;
    // Rebuild on the empty<->non-empty boundary so the placeholder hides
    // when the user types and reappears if the field is cleared.
  ...
  void _onTextChange() {
    if (!mounted) return;
    // Only rebuild on the empty <-> non-empty boundary; we don't need to
    // rebuild on every character once the placeholder is gone.
    final isEmpty = _controller.text.isEmpty;
    final placeholderShown = widget.placeholder != null && isEmpty;
    if (placeholderShown != _placeholderShown) {
      setState(() => _placeholderShown = placeholderShown);
    }
  }
