...
// Mobile build: desktop_drop is desktop-only (macOS/Windows/Linux file drop
// from the OS). On iOS/Android the system-drop affordance does not exist;
// in-pane DragTarget still works for Local↔Remote drag-and-drop, but the
// OS file-drop branch below is compiled out.
...
class _SftpDropTargetPaneState extends ConsumerState<SftpDropTargetPane> {
  ...
  Widget build(BuildContext context) {
    ...
    // System file drag-in (DropTarget) is desktop-only — skipped on mobile.
    return content;
  }
  ...
  // _handleSystemDrop removed in the mobile build — see desktop_drop comment
  // at the top of this file. The platform OS does not deliver file drops to
  // an iOS/Android app from outside its sandbox.
}
