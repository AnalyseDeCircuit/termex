    ...
    return Container(
      ...
      child: ListView(
        children: [
          ...
          Text(AppLocalizations.of(context).gitSyncExtraReposTitle,
              style: TextStyle(
                fontSize: 14,
          ...
          OutlinedButton.icon(
            ...
            label: Text(AppLocalizations.of(context).gitSyncAddRepo, style: const TextStyle(fontSize: 12)),
          ),
        ],
      ),
    );
  ...
  Future<void> _showAddRepoDialog(BuildContext context, WidgetRef ref) async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: Text(AppLocalizations.of(context).gitSyncAddDialogTitle),
        content: SizedBox(
          width: 400,
          child: Column(
            ...
            children: [
              TextField(
                ...
                decoration: InputDecoration(labelText: AppLocalizations.of(context).gitSyncLocalPath),
              ),
              TextField(
                controller: remoteCtrl,
                decoration: InputDecoration(labelText: AppLocalizations.of(context).gitSyncRemoteUrl),
              ),
              const SizedBox(height: 12),
              Row(
                mainAxisSize: MainAxisSize.min,
        ...
        actions: [
          ...
          ElevatedButton(
              ...
              child: Text(AppLocalizations.of(context).gitSyncAdd)),
        ],
      ),
    );
...
class _PrimaryStatusHeader extends ConsumerWidget {
            ...
            children: [
              ...
              if (status.enabled)
                TextButton.icon(
                  ...
                  label: Text(AppLocalizations.of(context).gitSyncManualSync, style: const TextStyle(fontSize: 12)),
                )
              else
                TextButton.icon(
                  onPressed: () => _showEnableDialog(context, ref),
                  icon: const Icon(Icons.play_arrow, size: 14),
                  label: Text(AppLocalizations.of(context).gitSyncEnable, style: const TextStyle(fontSize: 12)),
                ),
            ],
          ),
          if (status.enabled) ...[
            ...
            if (status.conflicts.isNotEmpty) ...[
              ...
              OutlinedButton.icon(
                ...
                label: Text(AppLocalizations.of(context).gitSyncResolveConflicts(status.conflicts.length.toString()),
  ...
  Future<void> _showEnableDialog(BuildContext context, WidgetRef ref) async {
    ...
    final ok = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text(AppLocalizations.of(context).gitSyncEnableDialogTitle),
        content: SizedBox(
          width: 400,
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              TextField(
                controller: localCtrl,
                decoration: InputDecoration(labelText: AppLocalizations.of(context).gitSyncLocalRepoPath),
              ),
              TextField(
                controller: remoteCtrl,
                decoration: InputDecoration(labelText: AppLocalizations.of(context).gitSyncRemoteUrlGitSsh),
              ),
            ],
          ),
        ),
        actions: [
          ...
          ElevatedButton(
              onPressed: () => Navigator.pop(ctx, true),
              child: Text(AppLocalizations.of(context).gitSyncEnable)),
        ],
      ),
    );
    if (ok == true && localCtrl.text.isNotEmpty && remoteCtrl.text.isNotEmpty) {
